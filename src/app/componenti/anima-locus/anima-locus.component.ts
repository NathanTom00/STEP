...
export class AnimaLocusComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute,private router : Router,private firestoreService: FirestoreService,protected authService:AuthService,private dialog:MatDialog) {}

  ngOnInit(): void {
    this.idLuogo = this.route.snapshot.paramMap.get('id_luogo');
    if(!this.route.snapshot.paramMap.get('iObiettivo') || this.idLuogo === null){
  ...
  openDialog(){
    this.dialog.open(AggiungiAnimaComponent,{data: {idLuogo: this.idLuogo,iObiettivo: this.iObiettivo}})
  }
  ...
  zoomImm(src:string){
    this.dialog.open(ZoomInImgComponent,{data: {src : src}});
  }
}
